...
module Kernel (Var, Atom, RecLit, PVal(..), Stmt(..), SkipType(..), fvar_pval, fvar_stmt, fvar_stmts, showStmts, l2s, forceRecompile) where
...
instance Show PVal where
  ...
  show (PProc vs stmts) = "proc {$ " ++ unwords vs ++ " } in\n" ++ indent (showStmts stmts) ++ "end"

instance Show Stmt where
  show (Skip Basic) = "skip Basic"
  show (Skip Store) = "skip Store"
  show (Skip Full)  = "skip Full"
  show (Skip Check) = "skip Check"
  show (Skip Stack) = "skip Stack"
  show (Skip (Browse s)) = "skip Browse " ++ s
  show (Thread stmts) = "thread in\n" ++ indent (showStmts stmts) ++ "end"
  show (Local vs stmts) = "local " ++ unwords vs ++ " in\n" ++ indent (showStmts stmts) ++ "end"
  show (EqVar v1 v2) = v1 ++ " = " ++ v2
  show (EqVal v1 val) = v1 ++ " = " ++ show val
  show (If v trues falses) = "if " ++ v ++ " then\n" ++ indent (showStmts trues) ++ "else\n" ++ indent (showStmts falses) ++ "end"
  show (Case v rl thens elses) = "case " ++ v ++ " of " ++ show_rec rl ++ " then\n" ++ indent (showStmts thens) ++ "else\n" ++ indent (showStmts elses) ++ "end"
