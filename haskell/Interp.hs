...
equality_check s@State{sas=sas,newlocs=newids} [s1,s2,s3] = case slookup s1 sas of
  ...
  where
    ...
    eq (SRec (l1,f1)) (SRec (l2,f2)) _ _ = 
      if l1 /= l2 || length f1 /= length f2 then Left False
      else 
        let (feats1, vals1) = unzip f1
            (feats2, vals2) = unzip f2
        in if feats1 /= feats2 then Left False
           else foldr (\(v1, v2) acc -> 
                  case acc of
                    Left False -> Left False
                    Right res -> Right res
                    Left True -> eq (snd (getVal v1)) (snd (getVal v2)) v1 v2) (Left True) (zip vals1 vals2)
      where getVal sloc = case slookup sloc sas of
              Nothing -> error $ "Impossible: store missing id " ++ show sloc
              Just (_,v) -> (sloc,v)
...
sched q n p s@State{front = (tn,[]):ths} = sched q 1 p s{front = ths}
...
sched q n p s@State{front = (tn,st@((m,e):st')):ths, back = back, sas = sas,
                    newthread = newt}
  = case exec m e s'' of
      EError msg -> SError msg
      ESuspend -> sched q 1 p s{front = ths, back = (tn,st):back}
      ENeed id id2 ->
...
showStmtType :: Stmt -> String
showStmtType (Skip _) = "Skip"
showStmtType (Thread _) = "Thread"
showStmtType (Local _ _) = "Local"
showStmtType (EqVar v1 v2) = "EqVar " ++ v1 ++ " " ++ v2
showStmtType (EqVal v _) = "EqVal " ++ v
showStmtType (If v _ _) = "If " ++ v
showStmtType (Case v _ _ _) = "Case " ++ v
showStmtType (NewName v) = "NewName " ++ v
showStmtType (Apply f _) = "Apply " ++ f
showStmtType (IsDet v1 v2) = "IsDet " ++ v1 ++ " " ++ v2
showStmtType (NewCell v1 v2) = "NewCell " ++ v1 ++ " " ++ v2
showStmtType (Exchange v1 _ _) = "Exchange " ++ v1
showStmtType (ByNeed v1 v2) = "ByNeed " ++ v1 ++ " " ++ v2
showStmtType (TryCatch _ v _) = "TryCatch " ++ v
showStmtType (Raise v) = "Raise " ++ v

showQueue :: [Thread] -> String
showQueue = show . map fst
